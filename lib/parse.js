...
ParseHTTPClient.prototype.postAsync = function(url, data, content_type, content_encoding, encode_json, cb){
	var self = this;
	content_type = content_type || 'application/json';
	content_encoding = content_encoding || undefined;
	encode_json = encode_json || true;
    ...
    var uri = self._server + url;
    request_async.post({uri: uri, "headers": headers, timeout: 5000, body: data_bytes}, function(err, response,body){
		try{
	    	response.body = JSON.parse(response.body);
	    }catch(e){
	    	response.body = response.body;
...
ParseHTTPClient.prototype.postFile = function(url, filename, content_type, content_encoding,encode_json, cb){
	var self = this;
	content_type = content_type || 'application/json';
	content_encoding = content_encoding || undefined;
	encode_json = encode_json || true;
    ...
    var uri = self._server + url;
	fs.createReadStream(filename).pipe(request_async.post({uri: uri, "headers": headers, timeout: 5000}, function(err, response,body){
		try{
	    	response.body = JSON.parse(response.body);
	    }catch(e){
	    	response.body = response.body;
...
StageHandler.prototype.upload_source_files = function(dir, suffixes, endpoint, old_checksums, old_versions, print_if_changes, callback){
    ...
    self.compute_checksums(source_files, function(checksums){
    	var queue = async.queue(function(task,complete){
    		var source_file = task.source_file;
	    	...
	    	if (old_checksums[filename] && old_checksums[filename] == checksums[source_file] && old_versions[filename]) {
	    		...
	    		//var source = fs.readFileSync(source_file, {encoding: 'utf8'});
		      	...
		      	changes_found = true;
		      	//self._output_file.write('Uploading %s...\n\n', source_file);
		      	self._http_client.postFile(endpoint+ '/' +encodeURI(filename), source_file, file_type, 'utf8', false, function(error, response){
			      	...
			      	versions[filename] = result['version'];
			      	self._output_file.write("%s/%s\r", queue.length()+"", source_files.length+"")
			      	if (err) {
			      		return complete(err);
			      	}else{
			      		return complete();
    	...
    	for(var i in source_files){
	    	...
	    	queue.push({source_file:source_file}, function(err){
	    		if (err) {
	    			queue.kill();
	    		};
	    	});
	...
	async.series([
		function(cb){
			self.upload_source_files('cloud', ['js', 'ejs', 'jade'], '1/scripts', old_script_checksums, old_script_versions, for_develop, function(err, res){
				if (err) {
					self._output_file.write("error... %s\n", err);
				}else{
					found_changes = res[2];
				}
		...
		function(err, results){
		    ...
		    response = self._http_client.postAsync('1/deploy', args, undefined, undefined, undefined,function(err, response){
			    ...
			    if (response.error || !response['releaseName']) {
